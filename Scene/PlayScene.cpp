...
#include "Turret/ShotgunTurret.hpp"
...
{
	playTime = (duration_cast<seconds>(high_resolution_clock::now() - gameStartTime)).count();
	// std::cout << "hmm...\n";
	AudioHelper::StopBGM(bgmId);
	// std::cout << "fishy...\n";
	AudioHelper::StopSample(deathBGMInstance);
	// std::cout << "fishy indeed...\n";
...
void PlayScene::Update(float deltaTime)
{
	...
	for (int i = 0; i < SpeedMult; i++)
	{
		...
		case 4:
			...
			break;
		case 5:
			EnemyGroup->AddNewObject(enemy = new Enemy5(SpawnCoordinate.x, SpawnCoordinate.y));
			break;
...
void PlayScene::UpdateScoreText()
{
...
void PlayScene::ReadMap()
{
	...
	for (int i = 0; i < MapHeight; i++)
	{
		for (int j = 0; j < MapWidth; j++)
		{
			...
			{
				std::string fileName = "play/dirt/";
				//* false --> dirt
				//* true --> floor
				bool top = i - 1 >= 0 ? mapData[(i - 1) * MapWidth + j] : true;
				bool bottom = i + 1 < MapHeight ? mapData[(i + 1) * MapWidth + j] : true;
				bool left = j - 1 >= 0 ? mapData[i * MapWidth + (j - 1)] : true;
				bool right = j + 1 < MapWidth ? mapData[i * MapWidth + (j + 1)] : true;
				...
				{
					//* Default
					fileName = "play/dirt.png";
				}

				TileMapGroup->AddNewObject(new Engine::Image(fileName, j * BlockSize, i * BlockSize, BlockSize, BlockSize));
			}
		}
	}
}
...
void PlayScene::ConstructUI()
{
	...
	UIGroup->AddNewObject(UIScore = new Engine::Label(std::string("Score: ") + std::to_string(gameScore), "pirulen.ttf", 24, 1294, 500));
	TurretButton *btn;
	// Button 1
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
						   ...
						   Engine::Sprite("play/turret-3.png", 1446, 136, 0, 0, 0, 0), 1446, 136, MissileTurret::Price);
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
						   ...
						   Engine::Sprite("play/turret-7.png", 1294, 212, 0, 0, 0, 0), 1294, 212, ShotgunTurret::Price);
...
void PlayScene::UIBtnClicked(int id)
{
	...
	else if (id == 3 && money >= ShotgunTurret::Price)
	{
		preview = new ShotgunTurret(0, 0);
	}

	if (preview == nullptr)
	{
		printf("set preview to null\n");
		return;
	}
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance()
{
	...
	std::vector<std::vector<bool>> visited(MapHeight, std::vector<bool>(std::vector<bool>(MapWidth, false)));
	// Push end point.
	// BFS from end point.
	...
	int r = MapHeight - 1, c = MapWidth - 1;
	visited[MapHeight - 1][MapWidth - 1] = true;

	Engine::Point bottom_right = Engine::Point(MapWidth - 1, MapHeight - 1);
	int stop = 280;
	...
	while (!que.empty())
	{
		...
		// 	map[p.y][p.x] = r.Magnitude();

		if (p.y > 0 && mapState[p.y - 1][p.x] == TILE_DIRT && !visited[p.y - 1][p.x])
		{
			que.push(Engine::Point(p.x, p.y - 1));
			map[p.y - 1][p.x] = map[p.y][p.x] + 1;
		}
		if (p.y < MapHeight - 1 && mapState[p.y + 1][p.x] == TILE_DIRT && !visited[p.y + 1][p.x])
		{
			que.push(Engine::Point(p.x, p.y + 1));
			map[p.y + 1][p.x] = map[p.y][p.x] + 1;
		}
		if (p.x > 0 && mapState[p.y][p.x - 1] == TILE_DIRT && !visited[p.y][p.x - 1])
		{
			que.push(Engine::Point(p.x - 1, p.y));
			map[p.y][p.x - 1] = map[p.y][p.x] + 1;
		}
		if (p.x < MapWidth - 1 && mapState[p.y][p.x + 1] == TILE_DIRT && !visited[p.y][p.x + 1])
		{
			que.push(Engine::Point(p.x + 1, p.y));
			map[p.y][p.x + 1] = map[p.y][p.x] + 1;
		}
