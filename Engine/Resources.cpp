...
namespace Engine
{
	...
	std::shared_ptr<ALLEGRO_BITMAP> Resources::GetBitmap(std::string name)
	{
		...
		if (!bmp)
			throw Allegro5Exception(("failed to load image: " + bitmapPath).c_str());
	...
	std::shared_ptr<ALLEGRO_BITMAP> Resources::GetBitmap(std::string name, int width, int height)
	{
		...
		if (!resized_bmp)
			throw Allegro5Exception(("failed to create bitmap when creating resized image: " + bitmapPath).c_str());
